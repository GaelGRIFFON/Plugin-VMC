...
try {
    ...
    if (init('action') == 'getAll') {
        $eqLogics = eqLogic::byType('vmc'); // ne pas oublier de modifier pour le nom de votre plugin
    ...
    if (init('action') == 'saveStack') {
        $params = init('params');
        ajax::success(vmc::saveStack($params)); // ne pas oublier de modifier pour le nom de votre plugin
    }
